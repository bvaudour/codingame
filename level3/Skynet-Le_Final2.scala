...
object Player extends App {
  ...
  //   - dgrLevel : Niveau de danger du nœud
  class Analyse(val id: Noeud, var distance: Int = Int.MaxValue, var parent: Option[Noeud] = None, var dgrLevel: Int = 0) {

    // Définit le nœud précédent
    def setParent(a: Analyse): Analyse = {
      parent   = Some(a.id)
      distance = a.distance + 1
      if (!id.passerelle) dgrLevel = id.countP + a.dgrLevel - 1
      this
    }
  ...
  def mostDangerousLink(distances: HashMap[Noeud, Analyse], gateways: HashSet[Noeud]): (Noeud, Noeud) = {
    val g    = gateways.toSeq.sortBy(n => distances(n).distance)
    val dgrs = distances.filter(e => e._1.countP > 1).toSeq.sortBy(_._2.distance)
    if (distances(g(0)).distance <= 1 || dgrs.isEmpty) return (distances(g(0)).prev, g(0))
    for ((n, d) <- dgrs; if (d.dgrLevel > 0)) {
      return (n, n.passerelles.minBy(p => distances(p).distance))
    }
    val n = dgrs(0)._1
    (n, n.passerelles.minBy(p => distances(p).distance))
  }
